...
struct COMMAND{
    char *name;
    void (*pfunc) (/*char * tr[], int att, ArrayList A, tListH L*/) ;   //specified inside prcess input
...
void CmdHist(char * tr[], int numtr,ArrayList A, HistList * L){
...
void Cmd_AsignarMalloc (char * tr[], ArrayList A){
    if (tr[0] == NULL) 
    { printList(A, "malloc"); return; }
    else {
        ...
        insertItem(A, p, size, timec, "malloc", 0, "", 0);
    }
}

    void * MmapFichero (char * file, int protection, ArrayList A){
        ...
        insertItem(A, p, s.st_size, timec, "mmap", df, file, 0);
        return p;
    }
/* 
...
*/
void Cmd_AsignarMmap (char * tr[], ArrayList A){
    char * perm;
    void * p;
    int protection = 0;
...
void * ObtenerMemoriaShmget (key_t key, size_t tam, ArrayList A){
    void * p;
    ...
    insertItem (A, p, tam, timec, "shared memory", 0, "", key);
    return (p);

    
}

void Cmd_AsignarCreateShared (char * tr[], ArrayList A){
...
void Cmd_AsignarShared (char * tr[], ArrayList A){
...
void CmdAsignar (char * tr[], int size, ArrayList A){
...
void Cmd_DesasignarMalloc (char * tr[], ArrayList A){
    ...
    {   /*Prints list of addresses allocated with -malloc;*/ 
        ...
        deleteElement(A, size, 0, 0, "malloc");
        return;    
    }
    
}

void Cmd_DesasignarMmap (char * tr[], ArrayList A){
    if ( (tr[0] == NULL) )
    {   /* Prints the list of addresses allocated with -mmap;*/
        printList(A, "mmap");
        return;
    }
    else {

        if ( deleteElement(A, 0, tr[0], 0, "mmap") ) {return;}
        else
        {   /* Prints the list of addresses allocated with -mmap;*/
            printList(A, "mmap");
            return;
...
void Cmd_DesasignarShared (char * tr[], ArrayList A){
    return;
}

void Cmd_DesasignarAddress (char * tr[], ArrayList A){
    int pos = findAddress (A, tr[0]);
    char * filter = "";
     strcpy (filter, (A)->nodes[pos].typeOfAllocation);
...
void CmdDesasignar (char * tr[], int size, ArrayList A){
...
void ProcessInput (char * inp, HistList * L, ArrayList A)
{
    char *tr[MAXLINE/2];
    int i;
    int numtr= 0;
...
int main (int argc, char *argv[])
{
    ...
    HistList L;
    ArrayList A;
    if (createListH(&L) == 0) {return 0;} //Hist list
    if (createList(&A) == 0 ) {return 0;} //Addresses´ list
